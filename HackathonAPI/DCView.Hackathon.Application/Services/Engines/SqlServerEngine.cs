...
/// <summary>
/// SQL Server implementation of the database engine abstraction.
/// </summary>
public class SqlServerEngine : IDbEngine
{
    ...
    {
        string dbName = $"{config.DbPrefix}{userId}";
        string loginName = $"hack_{userId}";
        string loginPassword = GenerateRandomPassword();

        string adminConnStr = $"Server={config.ServerName};Database=master;User Id={config.AdminUserId};Password={adminPassword};TrustServerCertificate=True;Connection Timeout=30;";
        ...
        using var userDbConn = new SqlConnection($"Server={config.ServerName};Database={dbName};User Id={config.AdminUserId};Password={adminPassword};TrustServerCertificate=True;Connection Timeout=30;");
    ...
    {
        string connStr = $"Server={config.ServerName};Database={databaseName};User Id={config.AdminUserId};Password={adminPassword};TrustServerCertificate=True;Connection Timeout=60;";
        ...
        for (int i = 0; i < batches.Count; i++)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        BatchIndex = i,
                        Type = queryType is "INSERT" or "UPDATE" or "DELETE" ? "DML" : "DDL",
    ...
    {
        string loginName = $"hack_{session.UserId}";
        return $"Server={config.ServerName};Database={session.DatabaseName};User Id={loginName};Password={loginPassword};TrustServerCertificate=True;Connection Timeout={config.MaxQueryTimeoutSeconds};";
    }

    public string? ValidateBatchSafety(string batch)
    {
        return SqlBatchParser.ValidateBatchSafety(batch);
    }

    public List<string> SplitBatches(string sql)
    {
        return SqlBatchParser.SplitBatches(sql);
    }
    ...
    public async Task<DatabaseOverviewDto> GetOverviewAsync(string connectionString)
    {
        ...
        var overview = new DatabaseOverviewDto { DatabaseName = conn.Database };
